...
> **Status: deferred to V2.** The finale is **not part of the V1 release**. Keep the supporting schema (`games.grand_finale`, the `game_state` finale anchors, `finale_answers`) so V2 can build on it without a migration, but in V1 **the feature is off everywhere**. (Note: the finale timer has no dedicated sound cue — its supported cue set is the same six shared by the rest of the game.)
...
**Sounds:** `sound_sets` (id, name) and `sounds` (sound_set_id, cue `correct`|`strike`|`round_start`|`round_end`|`game_start`|`end_game`, file_path). **Seed the three named packs** — Klasyczny, Retro, Modern — so the editor picker and the Dźwięki sub-tabs map onto real rows (see §10, change 4).
...
HTML5 `<audio>` in the browser. A configurable **sound set per game**, chosen in the Administrator game editor. Three packs ship (seeded in `db/schema.sql`): **Klasyczny / Retro / Modern**. Cues: correct answer (ding), strike/buzzer, round start, answer reveal, finale timer *(V2)*, end-of-game. If a cue has no uploaded file, the **default sound** at `assets/sounds/default/<cue>.wav` plays. Starter WAV cues ship for `default/`, the Klasyczny pack, and the Modern pack (Modern reuses the `default/` WAVs as its own explicit files) so the app makes sound out of the box (see `assets/sounds/README.md`); Retro starts empty and falls back to default.
